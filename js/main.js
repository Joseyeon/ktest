...
function mainvisualHeight(){
	...
	var visualH = $(window).height() - topnoticeH;
...
function dclogslick(){
	...
	slidediv.on('beforeChange', function(event, slick, currentSlide, nextSlide){ //슬라이드 변경 시 내비 및 페이징 변경
		//내비 변경
		if(enableNav){
		slidenav.parent('li').removeClass('on');
		slidenav.parent('li').eq(nextSlide).addClass('on');
		var currentTit = $('.dclogslide-for .item').eq(nextSlide).find('.tit').text();
